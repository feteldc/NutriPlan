...
import { Menu } from '../types/index.js';
...
export const generarMenuSemanal = async (datosUsuario: MenuInput): Promise<Menu> => {
  try {
    ...
    try {
      ...
      const comidas = ['desayuno', 'almuerzo', 'cena', 'snacks'] as const;
      
      for (const dia of dias) {
        if (!menu[dia]) throw new Error(`Falta el día ${dia}`);
        for (const comida of comidas) {
          if (!menu[dia][comida]) throw new Error(`Falta ${comida} en ${dia}`);
        }
      }
